...
TablColonNode* structure(string strTable, int i){
...
void Insert(string comand, configuration cond){
   ...
   //номер сsv
    ...
    out<<element;
    //Добавочные элементы
    for(int i=0; comand[i]!='\0'; i++){
        string data="";
        if(comand[i]=='\''){
            for(int j=i+1; comand[j]!='\'';j++){
                data+=comand[j];
                i=j;
            }
            i+=2;
